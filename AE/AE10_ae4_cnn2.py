...
def autoencoder(hidden_laysey_size):
    ...
    model.add(Conv2D(hidden_laysey_size, (2, 2),padding='valid' ,input_shape=(28, 28, 1)))
    model.add(MaxPool2D())
    model.add(Conv2D(hidden_laysey_size, (2, 2),
                     padding='valid'))
    model.add(MaxPool2D())
    
    model.add(Conv2DTranspose(1, (2,2), padding='valid'))
    model.add(UpSampling2D(
        size=(4, 4), data_format=None, interpolation='nearest'))
