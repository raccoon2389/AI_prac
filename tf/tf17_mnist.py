...
x_train, x_test = x_train.reshape(-1, 28*28) / \
    255., x_test.reshape(-1, 28*28)/255.
print(y_train[-1])
...
W1 = tf.get_variable("w1", [x_train.shape[1], layer_node[0]],
...
with tf.Session() as sess:
    ...
    a, l = sess.run([acc, loss], feed_dict={
                    x: x_test, y: y_test, rate: 0.1})
    print("test : ", a, l)
'''