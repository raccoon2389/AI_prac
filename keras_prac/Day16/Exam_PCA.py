...
samsung = pd.read_csv('./data/Samsung.csv',index_col=0 , header=0 , sep=',',encoding='euc-kr')
# print(samsung.head())
hite = pd.read_csv('./data/Hite.csv',index_col=0 , header=0 , sep=',',encoding='euc-kr')
# sns.heatmap(hite.corr().abs())

df = pd.merge(samsung,hite,how='outer',left_index=True,right_index=True).replace(',','',regex=True).dropna().astype('float')
df.columns=['samsung','hite','max','min','fin','amount']
print(df)
sns.heatmap(df.corr().loc['samsung':'hite','hite':'amount'])
plt.show()
