...
from keras.layers import Input, Dropout, LSTM,Dropout, Dense,LeakyReLU
import numpy as np
...
def build_model(drop,opt,lr,act):
    ...
    x = Dropout(drop)(x)
    output1 = Dense(10,name='output',activation=act)(x)
...
def create_hyper():
    ...
    act = ['relu','elu','selu',LeakyReLU()]
    return{"batch_size" : batches,'opt':optimizer ,"drop": droptout,'lr' : lr,'act':act}
