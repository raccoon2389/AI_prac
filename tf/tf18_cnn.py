...
x_train, x_test = x_train.reshape(-1, 28,28,1) / 255.\
    , x_test.reshape(-1, 28,28,1)/255.
print(y_train[-1])
...
layer11 = tf.nn.max_pool(layer1,ksize=[1,2,2,1],strides=[1,1,1,1],padding='SAME')
...
W2 = tf.get_variable("w2", shape = [3,3,1,32],
                     initializer=tf.contrib.layers.xavier_initializer())
layer2 = tf.nn.relu(tf.nn.conv2d(layer11, W2, strides=[1, 1, 1, 1], padding='SAME'))
layer22 = tf.nn.max_pool(layer1,ksize=[1,2,2,1],strides=[1,1,1,1],padding='SAME')
